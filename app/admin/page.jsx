...
export default function AdminPage() {
  ...
  const accumulate = (rows) => {
    const totals = rows.reduce((a,r)=>{
      a.spend += r.spend||0; a.revenue += r.revenue||0; a.orders += r.orders||0; return a;
  ...
  const buildSchema = (rows) => {
    ...
    const isWide = ["total_ad_spend","shopify_revenue","meta_amount_spent",
      "meta_website_purchases_conversion_value","meta_website_purchase_roas","meta_website_purchases",
      "google_cost","google_conv_value","google_conv_value_per_cost","google_conversions"
    ].every(h => headers.includes(h));

    // WIDE -> buckets
    if (isWide) {
      const overall = [], google = [], meta = [];
      rows.forEach(r => {
        const rr = Object.fromEntries(Object.entries(r).map(([k,v])=>[k.toLowerCase(), v]));
        const d = parseDate(rr.date);
        if (!d) return;
        const toF = (x) => toFloat(rr[x]);
        const toI = (x) => isNaN(parseInt(rr[x])) ? 0 : parseInt(rr[x]);
        const mSpend = toF("meta_amount_spent"), mRev = toF("meta_website_purchases_conversion_value"), mOrders = toI("meta_website_purchases");
        const gSpend = toF("google_cost"), gRev = toF("google_conv_value"), gOrders = toI("google_conversions");
        const totalSpend = toF("total_ad_spend"), shopRev = toF("shopify_revenue");
      ...
      const accumulate = (arr) => arr.reduce((a,r)=>({spend:a.spend+(r.spend||0),revenue:a.revenue+(r.revenue||0),orders:a.orders+(r.orders||0)}),{spend:0,revenue:0,orders:0});
      const totOverall = accumulate(overall); totOverall.roas = totOverall.spend ? +(totOverall.revenue/totOverall.spend).toFixed(2) : 0;
      const totMeta = accumulate(meta); totMeta.roas = totMeta.spend ? +(totMeta.revenue/totMeta.spend).toFixed(2) : 0;
      const totGoogle = accumulate(google); totGoogle.roas = totGoogle.spend ? +(totGoogle.revenue/totGoogle.spend).toFixed(2) : 0;
      return { daily: overall, totals: totOverall, channels: { google: { totals: totGoogle, daily: google }, meta: { totals: totMeta, daily: meta } } };
    }
    ...
    const accumulate = (arr) => arr.reduce((a,r)=>({spend:a.spend+(r.spend||0),revenue:a.revenue+(r.revenue||0),orders:a.orders+(r.orders||0)}),{spend:0,revenue:0,orders:0});
    const totOverall = accumulate(buckets.overall); totOverall.roas = totOverall.spend ? +(totOverall.revenue/totOverall.spend).toFixed(2) : 0;
    const totGoogle = accumulate(buckets.google); totGoogle.roas = totGoogle.spend ? +(totGoogle.revenue/totGoogle.spend).toFixed(2) : 0;
    const totMeta = accumulate(buckets.meta); totMeta.roas = totMeta.spend ? +(totMeta.revenue/totMeta.spend).toFixed(2) : 0;
    return {
    ...
    <div className="p-6 md:p-10">
      <div className="mx-auto max-w-5xl space-y-6">
        <div className="flex items-center justify-between">
          ...
          <Link href="/" className="text-sm px-3 py-1.5 rounded-lg border bg-white/70 dark:bg-gray-900/60">Back</Link>
        </div>
        ...
        <div className="grid md:grid-cols-2 gap-4">
          <label className="rounded-xl border border-gray-300 dark:border-gray-800 bg-white/80 dark:bg-gray-900/60 p-4 space-y-2 cursor-pointer">
            ...
            <div className="text-xs text-gray-500">With or without <code>platform</code> column</div>
            <input type="file" accept=".csv,text/csv" className="hidden" onChange={(e)=>e.target.files?.[0] && handleCSV(e.target.files[0])} />
            <div className="text-xs mt-2 text-gray-600 dark:text-gray-400">Expected headers: <code>date, spend, revenue, orders[, platform]</code></div>
          </label>
          ...
          <label className="rounded-xl border border-gray-300 dark:border-gray-800 bg-white/80 dark:bg-gray-900/60 p-4 space-y-2 cursor-pointer">
            ...
            <input type="file" accept="application/json,.json" className="hidden" onChange={(e)=>e.target.files?.[0] && handleJSON(e.target.files[0])} />
          </label>
        </div>
        ...
        {jsonData && (
          <div className="space-y-3">
            <div className="flex gap-2">
              <button onClick={()=>download("data.json", JSON.stringify(jsonData, null, 2))}
                className="px-3 py-1.5 text-sm rounded-lg border bg-white/70 dark:bg-gray-900/60">
                Download data.json
              </button>
              <a href="https://github.com" target="_blank" className="px-3 py-1.5 text-sm rounded-lg border bg-white/70 dark:bg-gray-900/60">
...
{JSON.stringify(jsonData, null, 2)}
        ...
        <div className="text-xs text-gray-500">
          Tip: Commit your CSV into <code>data/</code> in your repo. The GitHub Action will auto-generate <code>public/data.json</code> and Vercel will redeploy.
        </div>
      </div>
    </div>
  );
